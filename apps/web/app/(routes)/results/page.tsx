    ...
    <div className="mx-auto max-w-7xl px-4 py-6 md:px-6 space-y-8 pb-16">
      ...
      <header className="flex justify-between items-center w-full py-4 border-b border-slate-200/60 dark:border-zinc-800/60">
        <div className="flex items-center gap-4">
          <Link 
            href="/" 
            className="p-2 rounded-xl border border-slate-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-900/80 hover:bg-slate-50 dark:hover:bg-zinc-800 shadow-sm text-slate-600 dark:text-zinc-400 transition-all"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Left Side: Summary, Health, Actions, Charts, Downloads */}
              ...
              <div className="md:col-span-2 flex flex-col justify-center space-y-4">
                ...
                {parsedHealth.issues.length === 0 ? (
                  ...
                  <ul className="space-y-2">
                    {parsedHealth.issues.map((issue, idx) => (
                      <li key={idx} className="flex items-start gap-2.5 text-sm text-slate-600 dark:text-zinc-400">
                ...
                {data.issues?.length > 0 && (
                  <button 
                    ...
                    className="mt-4 text-xs font-bold text-red-500 hover:text-red-600 uppercase tracking-wider text-left transition-colors"
                  >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {data.visual_insights.length === 0 ? (
                ...
                data.visual_insights.map((chart: any, index: number) => (
                  <div key={index} className="card p-4 flex flex-col justify-between hover:shadow-md transition-shadow">
                    <h4 className="title text-xs font-extrabold text-slate-500 dark:text-zinc-400 mb-3 uppercase tracking-wider">
                      {chart.title}
                    </h4>
        ...
        {/* Right Side Sticky: Chat panel & Download section */}
        <div className="space-y-8 lg:sticky lg:top-8 self-start">
          
          {/* SECTION 5: Dataset Copilot */}
          <section className="space-y-4">
            <h2 className="title text-lg font-bold text-slate-800 dark:text-zinc-100 flex items-center gap-2">
            ...
            <div className="card p-6 space-y-6">
              ...
              <div className="flex flex-col gap-3">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 rounded-xl bg-slate-900 dark:bg-zinc-50 hover:bg-slate-800 dark:hover:bg-zinc-200 text-white dark:text-zinc-950 py-3.5 text-sm font-extrabold shadow-md transition-all disabled:opacity-50"
                >
                  {downloading ? (
                    <>
                ...
                <button
                  ...
                  disabled={reporting}
                  className="w-full flex items-center justify-center gap-2 rounded-xl border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 hover:bg-slate-50 dark:hover:bg-zinc-800 text-slate-700 dark:text-zinc-300 py-3.5 text-sm font-extrabold shadow-sm transition-all disabled:opacity-50"
                >
                  {reporting ? (
                    <>
      ...
      {showAuditModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-zinc-900 rounded-2xl w-full max-w-4xl max-h-[80vh] flex flex-col shadow-2xl border border-slate-200 dark:border-zinc-800">
            <div className="p-6 border-b border-slate-100 dark:border-zinc-800 flex justify-between items-center">
              <h2 className="title text-xl font-bold text-slate-900 dark:text-zinc-50">Detailed Quality Audit</h2>
              <button onClick={() => setShowAuditModal(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-zinc-300">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
              </button>
            </div>
            
            {/* Tabs header */}
            <div className="px-6 py-2 bg-slate-50 dark:bg-zinc-900 border-b border-slate-100 dark:border-zinc-800 flex gap-4">
              <button 
                onClick={() => setActiveTab("findings")}
                className={`py-2 px-3 text-xs font-bold uppercase tracking-wider border-b-2 transition-all ${
                  activeTab === "findings" 
